...
function actualizarOpcionesParaAvion(menuAvion, claveSubVar)
    ...
    -- Aeropuertos válidos por bandera y cooldown
    ...
    -- Crear submenús por página si no existen
    for pagina = 1, totalPaginas do
        ...
        if not paginasPorAvion[claveSubVar][pagina] then
            local nuevoSub = missionCommands.addSubMenuForCoalition(2, nombreSubmenu, menuAvion)
            paginasPorAvion[claveSubVar][pagina] = nuevoSub
        end

        -- Limpiamos comandos anteriores
