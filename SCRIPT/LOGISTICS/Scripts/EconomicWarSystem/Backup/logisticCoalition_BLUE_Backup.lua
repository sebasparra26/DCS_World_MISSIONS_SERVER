...
namesBlue = {}
for i = 1, 9999 do table.insert(namesBlue, "USA air " .. i) end

local cooldownTiempo = 200 -- segundos

plantillasLogisticaBLUE = {
...
function cargarInventarioCompleto(nombreAeropuerto, data)
...
function ejecutarEntrega(aeropuerto, data, tipoAvion)
    if trigger.misc.getUserFlag(data.bandera) ~= 1 then
        trigger.action.outText("Aeródromo no disponible: " .. aeropuerto, 10)
        return
    end

    if menuCooldowns[aeropuerto] and timer.getTime() < menuCooldowns[aeropuerto] then
    ...
    local recargo = recargoAeropuerto[aeropuerto] or 1
    local costo = math.floor(baseCosto * recargo)

    if puntosCoalicion.PuntosAZUL < costo then
        trigger.action.outText("No tienes suficientes dólares. Requiere: $" .. costo, 10)
        return
    end

    local origen = { x = 202611, z = 7004 }
    local destino = coordenadasAerodromosBLUE[aeropuerto] or { x = 0, z = 0 }
    local dx, dz = destino.x - origen.x, destino.z - origen.z
    local distancia = math.sqrt(dx * dx + dz * dz)
    local velocidad = 138.88
    local tiempoEst = math.floor((distancia / velocidad) * (multiplicadorTiempo[aeropuerto] or 1))
    ...
    local nombresAntes = {}
    for _, nombre in ipairs(namesBlue) do
    ...
    timer.scheduleFunction(function()
        ...
        if grupoNuevo then
            ...
            menuCooldowns[aeropuerto] = timer.getTime() + cooldownTiempo
        end
    end, {}, timer.getTime() + 1)
end
            ...
            if grupo and Group.isExist(grupo) then
                ...
                if unidad then
                    ...
                    if info.altMax >= 914.4 and speed < 0.5 then
                        cargarInventarioCompleto(info.destino, tipoAviones[info.inventario])
                        info.entregado = true
                    end
                end
            end
