...
function actualizarOpcionesParaAvion(menuAvion, claveSubVar)
    -- Limpiar comandos previos (comando dummy para limpiar hijos)
    local dummyCommand = missionCommands.addCommandForCoalition(2, "_clean", menuAvion, function() end)
    missionCommands.removeItem(dummyCommand)

    local porPagina = 8
    local entradas = {}

    -- Filtrar aeropuertos con bandera activa (bandera = 1 para azules)
    ...
    for _, ref in ipairs(paginasPorAvion[claveSubVar]) do
        missionCommands.removeItem(ref)
    end
    paginasPorAvion[claveSubVar] = {}
    ...
    for pagina = 1, totalPaginas do
        ...
        table.insert(paginasPorAvion[claveSubVar], submenuPag)
        ...
        for i = 1, porPagina do
            ...
            if entrada then
                ...
                missionCommands.addCommandForCoalition(2, texto, submenuPag, function()
