-- ============================================
-- MENÚ LOGÍSTICO PARA COALICIÓN ROJA (RED)
-- ============================================
-- Este script crea un menú dinámico visible solo para los jugadores de la coalición roja.
-- Agrega las opciones de aeródromos controlados (bandera == 3) según el tipo de avión.
...
-- menuRaizPorGrupo: menú principal "Mercado de Pulgas" por grupo
...
-- ============================================
-- FUNCIÓN PRINCIPAL: crearMenuParaRojos
-- Detecta jugadores rojos activos y les crea el menú de entregas.
...
local function crearMenuParaRojos()
    -- Obtener unidades de jugadores Rojos
    local jugadoresRojos = mist.makeUnitTable({'[red][player]'})

    for _, unitName in ipairs(jugadoresRojos) do
        ...
        if unit then
            ...
            if group then
                ...
                if not menuRaizPorGrupo[groupID] then
                    menuRaizPorGrupo[groupID] = missionCommands.addSubMenuForGroup(groupID, "Mercado de Pulgas")
                ...
                for nombreAvion, claveTipo in pairs(tiposAvion) do
                    ...
                    for aeropuerto, data in pairs(plantillasLogistica) do
                        ...
                        -- Agregar la opción solo si bandera es 3 y no está en cooldown
                        if valorBandera == 3 and not cooldownActivo then
                            ...
                            if not itemsYaCreados[groupID][claveTipo][key] then
                                ...
                                missionCommands.addCommandForGroup(groupID, texto, menuAvion, function()
                                    ...
                                    if tipoAviones[claveTipo] then
                                        ejecutarEntrega(aeropuerto, data, claveTipo)
                                    else
