...
local unidadAWACS = {
  ["A-50"] = { type = "A-50", cs = {1, 1, 0}, tac = "AWC" }
}

local function MHz(v) return v * 1e6 end
local function randFreq() return MHz(math.random(2460, 2490) / 10) end
...
local function spawnAwacsRojo(tp, p1, p2, hdg)
...
function eventHandlerAwacs:onEvent(e)
  if e.id ~= world.event.S_EVENT_MARK_CHANGE or not e.text or not _G.__SEL_AWACS_ROJO then return end
  ...
  if t == "awacsh" or t == "awacsv" then
    ...
    spawnAwacsRojo(_G.__SEL_AWACS_ROJO, p1, p2, hdg)
    _G.__SEL_AWACS_ROJO = nil
  end
end
...
missionCommands.addCommandForCoalition(side, "AWACS Activos", rootMenuAWACS, function()
  ...
  for g, d in pairs(activeAwacsRojo) do
    if Group.getByName(g) and Group.getByName(g):isExist() then
...
local function resumenAutoAwacsRojo()
  local msg, now, hay = "AWACS Activos\n", timer.getTime(), false
  for g, d in pairs(activeAwacsRojo) do
    if Group.getByName(g) and Group.getByName(g):isExist() then
      local restante = math.max(0, math.floor(d.tiempoExpiracion - now))
  ...
  if not hay then msg = msg .. "(ninguno activo)" end
  trigger.action.outTextForCoalition(side, msg, 10)
  timer.scheduleFunction(resumenAutoAwacsRojo, {}, timer.getTime() + INTERVALO_RESUMEN)
end

timer.scheduleFunction(resumenAutoAwacsRojo, {}, timer.getTime() + 5)
