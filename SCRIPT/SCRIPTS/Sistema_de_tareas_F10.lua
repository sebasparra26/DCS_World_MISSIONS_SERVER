----------------------------------------------------------------
-- TASKING IA POR MARCAS F10
-- REQUIERE MIST CARGADO ANTES DE ESTE SCRIPT
--
-- COMANDOS DE MARCA:
--   cap
--   sead
...
--
-- NOTAS:
-- - Las plantillas deben estar en Late Activation
-- - Uncontrolled debe estar OFF
-- - Las plantillas salen desde tierra
-- - La marca F10 es el waypoint operativo
----------------------------------------------------------------

if not mist or not mist.cloneGroup then
    trigger.action.outText("ERROR: MIST no esta cargado. Carga MIST antes de este script.", 15)
    return
end
...
local function parseCommand(text)
    ...
    for i = 1, #COMMAND_ALIASES do
        ...
        if rawLower:sub(1, #alias + 1) == alias .. " " then
            local arg = trim(raw:sub(#alias + 2))
            return key, arg
        end
    end
...
local function canUseCategory(categoryKey)
    ...
    if #cat.activeTaskIds >= maxActive then
...
local function buildMissionTaskFromTemplate(templateName, profile, markPoint)
    local templateRoute = mist.getGroupRoute(templateName, true)
    if not templateRoute or not templateRoute[1] then
        return nil, "No se pudo leer la ruta base de la plantilla: " .. templateName
    end

    local wp1 = deepCopy(templateRoute[1])

    wp1.task = buildEmptyComboTask()
    wp1.name = wp1.name or "START"
    wp1.ETA = wp1.ETA or 0
    wp1.ETA_locked = false
    wp1.speed_locked = true

    local pointVec2 = makeVec2(markPoint)

    local wp2Alt = profile.orbitAltitude or profile.ingressAltitude or wp1.alt or 2000
    local wp2Speed = profile.orbitSpeed or profile.ingressSpeed or wp1.speed or 180
    local wp2AltType = wp1.alt_type or "BARO"
    ...
    local wp2 = {
        ...
        task = buildWaypointTaskForProfile(profile, pointVec2)
    }
...
local function assignTaskToClone(taskId)
    ...
    local missionTask, err = buildMissionTaskFromTemplate(rec.templateName, profile, rec.point)
    if not missionTask then
        rec.state = "ERROR: " .. (err or "no se pudo crear la Mission route")
        rec.finished = true
        releaseCategoryIfTaskFinished(rec.keyword, rec.id)
...
local function showAssignedTasks()
    ...
    local lines = {}
    lines[#lines + 1] = "TAREAS REGISTRADAS"
...
local function showCategoryStatus()
    ...
    local lines = {}
    lines[#lines + 1] = "ESTADO DE CATEGORIAS"
    ...
    for _, key in ipairs(keys) do
        ...
        if timer.getAbsTime() < (cat.nextAvailableAt or 0) then
...
local function showHelp()
    local text =
        ...
        "Notas:\n" ..
        "- La etiqueta F10 es el waypoint operativo\n" ..
        "- Las plantillas salen desde tierra\n" ..
...
trigger.action.outText(
    "Tasking IA cargado.\n" ..
    "La marca F10 ahora es el waypoint operativo.\n" ..
    "Comandos: cap, sead, cas, strike, ataque a tierra, escort NombreGrupo",
    12
)
