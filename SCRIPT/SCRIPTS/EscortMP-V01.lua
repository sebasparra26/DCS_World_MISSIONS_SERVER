...
EscortMP.cfg = {
    ...
    -- Posicion fisica inicial del spawn respecto al jugador
    spawnDerecha = 80,
    spawnAtras = 120,
    spawnVertical = 20,
    alturaSpawnEnSuelo = 300,

    -- Posicion relativa que usara la tarea Escort
    -- x = frente/atras, z = derecha/izquierda
    formacion = {
        x = -120,
        y = 0,
        z = 80
...
function EscortMP.msg(groupId, txt, t)
    if groupId then
        trigger.action.outTextForGroup(groupId, txt, t or 5)
    end
end
...
function EscortMP.buildMenuForUnit(unit)
    ...
    EscortMP.log("Menu creado para groupId " .. tostring(groupId) .. " jugador " .. tostring(playerName))
end
...
function EscortMP.getDbForUnit(unitName)
    if mist and mist.DBs then
        return mist.DBs.unitsByName[unitName] or mist.DBs.humansByName[unitName]
    end
    return nil
end
...
function EscortMP.makeSpawnPoint(playerUnit, category)
    ...
    if not playerUnit:inAir() then
        local ground = land.getHeight({ x = spawnX, y = spawnZ })
...
function EscortMP.makeEscortGroupData(playerUnit, playerGroup, groupId)
    ...
    local currentSpeed = math.sqrt(speedVec.x * speedVec.x + speedVec.y * speedVec.y + speedVec.z * speedVec.z)
    ...
    local spawnX, spawnZ, spawnAlt = EscortMP.makeSpawnPoint(playerUnit, category)
    local finalSpeed = math.max(currentSpeed, defaultSpeed)

    local groupName = string.format("ESCORT_%d_%d", groupId, math.floor(timer.getTime() * 100))
    ...
    local newGroup = {
        ...
        units = {
            [1] = {
                ...
                alt_type = "BARO",
                speed = finalSpeed,
                heading = mist.getHeading(playerUnit, true) or 0,
                payload = payload,
...
function EscortMP.applyEscortTask(groupId)
    ...
    EscortMP.log("Task Escort aplicada a " .. session.escortGroupName)
end
...
function EscortMP.scheduleDestroy(groupId, delay, reason)
    ...
    if session.destroyTaskId then
        return
    end

    session.destroyTaskId = mist.scheduleFunction(
        EscortMP.removeEscortNow,
        { groupId, reason, true },
        timer.getTime() + (delay or EscortMP.cfg.borrarTrasMuerteJugador)
    )
end
...
function EscortMP.removeEscortNow(groupId, reason, silent)
    ...
    if session.destroyTaskId then
        mist.removeFunction(session.destroyTaskId)
        session.destroyTaskId = nil
    end

    local escortGroup = Group.getByName(session.escortGroupName)
    if escortGroup and escortGroup:isExist() then
        Group.destroy(escortGroup)
    end
    ...
    EscortMP.log("Escolta removida para groupId " .. tostring(groupId))
end
...
function EscortMP.createEscort(groupId)
    ...
    local spawned = mist.dynAdd(groupData)
    if not spawned or not spawned.name then
        EscortMP.msg(groupId, "Fallo al spawnear la escolta.", 5)
        return
    end
    ...
    mist.scheduleFunction(EscortMP.applyEscortTask, { groupId }, timer.getTime() + 0.5)
...
function EscortMP.eventHandler:onEvent(event)
    ...
    if event.id == EVT.S_EVENT_PLAYER_ENTER_UNIT then
    ...
    if EVT.S_EVENT_PLAYER_LEAVE_UNIT and event.id == EVT.S_EVENT_PLAYER_LEAVE_UNIT then
    ...
    local groupId, session = EscortMP.getSessionByUnitName(unitName or "")
    if not session then
        return
    end

    if EVT.S_EVENT_LAND and event.id == EVT.S_EVENT_LAND then
    ...
    if event.id == EVT.S_EVENT_DEAD
    or event.id == EVT.S_EVENT_CRASH
    or event.id == EVT.S_EVENT_EJECTION
    or (EVT.S_EVENT_PILOT_DEAD and event.id == EVT.S_EVENT_PILOT_DEAD) then
...
mist.scheduleFunction(EscortMP.bootstrapMenus, {}, timer.getTime() + 1)
mist.scheduleFunction(
    EscortMP.housekeeping,
    {},
    timer.getTime() + EscortMP.cfg.chequeoSegundos,
    EscortMP.cfg.chequeoSegundos
)

EscortMP.log("EscortMP iniciado")