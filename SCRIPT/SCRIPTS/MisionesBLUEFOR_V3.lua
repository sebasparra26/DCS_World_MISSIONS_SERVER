...
local function checkAndSpawn()
    ...
    if activeGroup == nil or isGroupDead(activeGroup) then
        debug("No hay grupo activo o el actual ha muerto.")
        ...
        -- Verificar si todos los grupos han muerto
        ...
        -- Buscar el primer grupo disponible y válido
        for i = spawnStart, spawnEnd do
            local groupName = groupNamePrefix .. string.format("%02d", i)
            local flagID = groupFlags[groupName]
            if not isGroupDead(groupName) and (flagID == nil or trigger.misc.getUserFlag(flagID) ~= 2) then
                debug("Activando grupo válido: " .. groupName)
                spawnGroup(groupName)
                return
            end
        end

        debug("No se encontró ningún grupo válido para activar.")
...
-- Llamada periódica para verificar el estado del grupo activo
