local WHSYNC = {
    ...
    FILE_PATH = lfs.writedir() .. "Config\\HorizontDev\\SistemWarehousePersistanceCaucasus.json",
...
local function log(msg)
    ...
    if WHSYNC.DEBUG then
        trigger.action.outText("[WHSYNC] " .. msg, 8)
    end
end
...
local function shouldTrackAirbase(id)
    if WHSYNC.ONLY_AIRBASE_IDS == nil then
        return true
    end
    return contains(WHSYNC.ONLY_AIRBASE_IDS, id)
end
...
local function startupProbe()
    ...
    for id, ab in pairs(WHSTATE.airbasesById) do
        ...
        if snap then
            checked = checked + 1
            if checked <= 3 then
                log("Prueba airbase " .. tostring(id) .. " (" .. tostring(snap.name) .. ") leido correctamente")
            end
        end
    end

    log("Total de aeropuertos monitoreados: " .. tostring(checked))
end
...
local function validateEnvironment()
    ...
    if not net or not net.lua2json or not net.json2lua then
        log("net.lua2json/json2lua no disponible en esta misión.")
        return false
    end
